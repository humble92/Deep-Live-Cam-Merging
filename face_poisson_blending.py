...
def regional_swap_poisson(source_image_path: str, target_image_path: str, output_path: str):
    """
    Performs a regional face swap using Poisson blending.
    ...
    # Get the center of the masks
    eye_moments = cv2.moments(eye_mask)
    eye_center = (int(eye_moments["m10"] / eye_moments["m00"]), int(eye_moments["m01"] / eye_moments["m00"]))
    nose_moments = cv2.moments(nose_mask)
    nose_center = (int(nose_moments["m10"] / nose_moments["m00"]), int(nose_moments["m01"] / nose_moments["m00"]))
    mouth_moments = cv2.moments(mouth_mask)
    mouth_center = (int(mouth_moments["m10"] / mouth_moments["m00"]), int(mouth_moments["m01"] / mouth_moments["m00"]))

    # Combine the swapped images using Poisson blending
    final_image = target_image.copy()
    final_image = cv2.seamlessClone(swapped_image_1, final_image, eye_mask, eye_center, cv2.NORMAL_CLONE)
    final_image = cv2.seamlessClone(swapped_image_2_resized, final_image, nose_mask, nose_center, cv2.NORMAL_CLONE)
    final_image = cv2.seamlessClone(swapped_image_1, final_image, mouth_mask, mouth_center, cv2.NORMAL_CLONE)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Perform a regional face swap using Poisson blending.")
    ...
    regional_swap_poisson(args.mother_image, args.father_image, son_output_path)

    # Create the daughter image
    daughter_output_path = os.path.join(args.output_dir, "daughter.jpg")
    regional_swap_poisson(args.father_image, args.mother_image, daughter_output_path)
